...
$stmt->bindValue(':clientFirstname', $clientFirstname, PDO::PARAM_STR);
$stmt->bindValue(':clientLastname', $clientLastname, PDO::PARAM_STR);
$stmt->bindValue(':clientEmail', $clientEmail, PDO::PARAM_LOB);
$stmt->bindValue(':clientId', $clientId, PDO::PARAM_INT);
$stmt->execute();
$updateClient = $stmt->rowCount();
$stmt->closeCursor();
return $updateClient;
}

// Get client data based on an clientId
function getNewClient($clientId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT * FROM clients WHERE clientId = :clientId';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':clientId', $clientId, PDO::PARAM_STR);
    $stmt->execute();
    $newClientData = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
...
function updatePassword($clientPassword, $clientId) {
    ...
    $stmt->bindValue(':clientId', $clientId, PDO::PARAM_INT);
    $stmt->execute();
    $updateClientPassword = $stmt->rowCount();
    $stmt->closeCursor();
    return $updateClientPassword;
}